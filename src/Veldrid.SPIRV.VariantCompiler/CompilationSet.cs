...
public class VariantStageDescription(ShaderStages stage, string fileName)
{
    public ShaderStages Stage { get; } = stage;
    ...
    CrossCompileTarget[] targets)
{
    ...
    public VariantStageDescription[] Shaders { get; } = shaders;
    public MacroDefinition[] Macros { get; } = macros;
    public CrossCompileOptions CrossCompileOptions { get; } = crossCompileOptions;
    public CrossCompileTarget[] Targets { get; } = targets;
}

public class VariantCompiler
{
    private readonly List<string> _shaderSearchPaths;
    ...
    private string[] CompileVertexFragment(ShaderVariantDescription variant)
    {
        ...
        List<Exception> compilationExceptions = [];
        byte[]? vsBytes = null;
        byte[]? fsBytes = null;
    ...
    private static string GetExtension(CrossCompileTarget target) => target switch
    {
